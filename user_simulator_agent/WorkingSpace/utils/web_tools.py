...
class WebTools:
    def __init__(self, serper_key: str, jina_key: str, proxy: str = None):
    ...
    def search(self, query: str, num: int = 5) -> list[dict]:
        ...
        try:
            ...
            if self.proxy:
                # Use requests for proxy support instead of http.client
                ...
                conn = http.client.HTTPSConnection("google.serper.dev", timeout=15, context=ctx)
                conn.request(
                    "POST", "/search",
    ...
    def download_binary(self, url: str, save_path: str,
                        timeout: int = 180, max_mb: int = 200) -> bool:
        """
