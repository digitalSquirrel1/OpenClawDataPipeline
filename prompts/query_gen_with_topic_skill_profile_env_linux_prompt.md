...
- **鼓励你提出依赖本地文件才能解决的query**：可以对本地文件做读取、搜索、整理、汇总等操作。请注意，该用户使用的是Linux系统，文件路径遵循Linux风格。用户的主目录为 `~/`（home目录），其他存储卷挂载在 `~/X/` 下（X为卷标字母，如 `~/D/`、`~/E/` 等）。query中提及本地文件时，允许你不提及详细的绝对路径，给出文件名、文件的主要内容或者粗略的提及文件路径即可。例如，可以说"home目录下的...""~/D/目录里的...""D目录下面的..."等。请注意表述方式的多样性，不要总是同一种表达。在你生成的queries中，建议有80%+需要依赖读取本地文件，另外20%可以不依赖本地文件读取。
